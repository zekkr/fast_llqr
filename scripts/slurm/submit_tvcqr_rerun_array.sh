...
CHECK_OUT_DIR="${FASTQR_TVCQR_CHECK_OUT_DIR:-results/table}"
...
JOB_NAME="tvcqr_c${FASTQR_CASE}_tau${TAU_INT}_n${FASTQR_N}_rep${FASTQR_NUM_REP}_rerun"

cd "$PROJECT_DIR"

ARRAY_JOB_ID=$(sbatch <<EOINNER | awk '{print $4}'
#!/bin/bash
#SBATCH -J ${JOB_NAME}
#SBATCH -p ${PARTITION}
#SBATCH -A ${ACCOUNT}
...
MERGE_JOB_ID=$(sbatch --dependency=afterany:${ARRAY_JOB_ID} <<EOINNER | awk '{print $4}'
#!/bin/bash
...
CHECK_JOB_ID=$(sbatch --dependency=afterany:${MERGE_JOB_ID} <<EOINNER | awk '{print $4}'
#!/bin/bash
...
export LD_LIBRARY_PATH=/apps/soft/R/R-4.3.1/lib64:\$LD_LIBRARY_PATH
export R_LIBS_USER="/home/$USER/R/x86_64-pc-linux-gnu-library/4.3"
export FASTQR_TVCQR_CHECK_OUT_DIR=${CHECK_OUT_DIR}

cd ${PROJECT_DIR}
Rscript scripts/array/check_tvcqr_config_integrity.R
EOINNER
)
