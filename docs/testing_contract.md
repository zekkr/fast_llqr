...
- `init_mode`
- `init_trigger`
- `independent_init_count`
- `full_active_recovery_count`
- `H_seq_ppro`
- `H_seq_seq`
...
under the fit at `z[j]` is exactly zero. The last evaluation point has no
next-window count. Also report all-observation and current-window literal counts.
