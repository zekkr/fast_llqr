...
module weighted_qr_tree_core
  use iso_fortran_env, only: real64,int64
  use, intrinsic :: ieee_arithmetic
  implicit none
  private
  public :: fit_screen_tree
...
contains
  subroutine fit_screen_tree(a,y,n,q,ne,visit_order,parent,tau,tol,maxit,threshold,min_keep,cache_flags,get_weights, &
                             beta_out,h_out,diagnostics,ierr,failed_eval)
    integer,intent(in)::n,q,ne,visit_order(ne),parent(ne),maxit,min_keep,cache_flags
    ...
    integer::ev,step,node,pnode,na,ns,ms,i,j,k,ii,g,nb,attempt,iters,cold_status,solve_status,nh
    ...
    logical::reuse_raw_rows,full_recorded,use_active_lists,visited(ne)
    ...
    visited=.false.
    do step=1,ne
      node=visit_order(step)
      if(node<1 .or. node>ne .or. visited(node)) then
        ierr=5; failed_eval=max(0,node); return
      end if
      if(step==1) then
        if(parent(node)/=0) then
          ierr=5; failed_eval=node; return
        end if
      else
        pnode=parent(node)
        if(pnode<1 .or. pnode>ne .or. .not.visited(pnode)) then
          ierr=5; failed_eval=node; return
        end if
      end if
      visited(node)=.true.
    ...
    do step=1,ne
      node=visit_order(step); ev=node
      if(step>1) then
        pnode=parent(node)
        previous_beta=beta_out(pnode,:); hp=h_out(pnode,:)
        previous_bits=transfer(previous_beta,previous_bits)
        ! A tree edge may switch to a parent other than the node visited just
        ! before it. Cached residuals, inverse ownership and transformed rows
        ! therefore never cross edge ownership.
        residual_epoch=residual_epoch+1_int64
        inverse_owner=0; row_owner=0
        row_epoch=row_epoch+1_int64
      end if
      call get_weights(node,n,w,active_ids,na)
      diagnostics(ev,1)=na
  ...
  end subroutine fit_screen_tree
...
end module weighted_qr_tree_core
