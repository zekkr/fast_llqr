...
!
! Temporary v42 contract-aligned experiment: v33 plus an exact positive-weight
! row index for the bounded kernel. Later-point thresholding, retained-row
! packing, aggregation, and omitted-row verification traverse only w>0 rows;
! previous-H zero-weight rows are merged back in original observation order.
! The Gaussian kernel keeps v33's dense vectorizable loops. The first point
! remains the original full-m solve and all rowwise verification is retained.
!
