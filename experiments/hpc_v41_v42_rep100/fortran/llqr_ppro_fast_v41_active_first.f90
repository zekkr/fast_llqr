...
!
! Temporary v41 research experiment: v42's exact positive-weight row index plus
! a bounded-kernel first-point solve over only the exact w>0 rows.  Gaussian
! first points and later loops retain the dense v33 path because underflow-zero
! tail rows can affect the selected numerical slope/basis and indirect indexing
! inhibits vectorization. If the bounded active problem
! cannot provide a valid two-row interpolation basis, the original full-m first
! solve is run.  Removing exact zero-weight constraints preserves the binary
! weighted objective but can change the simplex route/basis in nonunique cases,
! and it therefore does not match the current full-sample-first contract verbatim.
!
...
subroutine llqr_ppro_fortran(x, y, z, m, nvar, rounds, tau, h, tol, maxit, &
    ...
    ! Fast no-history experiment: the default R interface discards residuals,
    ! so retain only the preceding accepted residual vector internally.
    ! Keep an ABI-compatible placeholder pointer for the legacy argument.
    ...
    logical :: bland, always_same_h_refit, threshold_lower_bound, z_is_sorted
    ...
    integer :: idx_not_jl_or_jh(m), active_ids(m)
    integer :: forced_h_ids(nvar+1), n_forced_h
    integer :: active_pos, forced_pos, next_id, insert_id, insert_pos
    logical :: duplicate_h
    integer :: n_subsample, n_potential_S
    integer :: min_subsample_size, min_subsample_size_effective
    integer :: n_active, target_min
    double precision, target :: residual_buffer_1(m), residual_buffer_2(m)
    double precision, pointer :: r(:), r_raw(:), r_candidate(:), residual_swap(:)
    double precision :: accepted_beta(nvar+1)
    integer(int64) :: accepted_beta_bits(nvar+1), trial_beta_bits(nvar+1)
    logical :: candidate_reuses_accepted_residual
    integer :: n_refit_attempts_local, n_beta_cache_hits_local
    integer :: n_residual_evals_avoided_local, n_cache_hit_accepts_local
    logical :: screen_topology_cache_valid, reuse_screen_topology
    integer(int64) :: cached_threshold_bits, current_threshold_bits
    integer :: cached_screen_H(nvar+1), n_topology_reuses_local
    ...
    failed_eval = 0
    r => residual_buffer_1
    r_raw => residual_buffer_2
    r_candidate => r_raw
    candidate_reuses_accepted_residual = .false.
    n_refit_attempts_local = 0
    n_beta_cache_hits_local = 0
    n_residual_evals_avoided_local = 0
    n_cache_hit_accepts_local = 0
    screen_topology_cache_valid = .false.
    reuse_screen_topology = .false.
    cached_threshold_bits = 0_int64
    current_threshold_bits = 0_int64
    cached_screen_H = 0
    n_topology_reuses_local = 0
    first_n_sub_out = 0
    repair_count_out = 0
    final_n_sub_out = 0
    init_mode_out = 0
    ...
    ! Research branch: solve the exact positive-weight pointwise LP.  Rows are
    ! packed in original observation order, so the active tableau is otherwise
    ! identical to the corresponding rows of the full-m cold start.
    first_active_success = (case_int == 2 .and. nvar == 1 .and. &
                            n_active >= nvar + 1)
    if (first_active_success) then
        do i = 1, n_active
            k = active_ids(i)
            ws(i) = w(k)
            b(i) = abs(y(k))
        ...
        rr = 0.0d0
        do j = 1, nvar+1
            gammax(n_active+1, j) = 0.0d0
            do i = 1, n_active
                if (IB(i) > nvar + 1 .and. &
                    IB(i) <= nvar + 1 + n_active) then
                    k = IB(i) - nvar - 1
                    gammax(n_active+1, j) = gammax(n_active+1, j) - &
                        tau * ws(k) * gammax(i, j)
                else if (IB(i) > nvar + 1 + n_active) then
                    k = IB(i) - nvar - 1 - n_active
                    gammax(n_active+1, j) = gammax(n_active+1, j) - &
                        (1.0d0 - tau) * ws(k) * gammax(i, j)
                end if
            end do
        end do

        call run_simplex_full_llqr(gammax, b, IB, freevarrow, r1, r2, rr, ws, &
                                   m+1, n_active, nvar, tau, tol, maxit, bland, &
                                   iter, no_pivot_flag, simplex_converged)
        first_active_success = (.not. no_pivot_flag) .and. simplex_converged
        if (first_active_success) then
            call extract_solution(gammax, b, IB, n_active, nvar, estimate, u, v, r1)
            do i = 1, nvar+1
                k = r1(i) - nvar - 1
                if (k < 1 .or. k > n_active) then
                    first_active_success = .false.
                else
                    H_candidate(i) = active_ids(k)
                end if
            end do
        end if
        if (first_active_success) then
            if (H_candidate(1) == H_candidate(2)) first_active_success = .false.
        end if
        if (first_active_success) then
            do i = 1, nvar+1
                do j = 1, nvar+1
                    xh(i, j) = A(H_candidate(i), j)
                end do
            end do
            if (abs(xh(1,1) * xh(2,2) - xh(1,2) * xh(2,1)) <= 1.0d-10) then
                first_active_success = .false.
            else
                call inv22(xh, xhinv, inv_success)
                if (.not. inv_success) first_active_success = .false.
            end if
        end if
        if (first_active_success) then
            do i = 1, nvar+1
                estimate(i) = 0.0d0
                do j = 1, nvar+1
                    estimate(i) = estimate(i) + xhinv(i, j) * y(H_candidate(j))
                end do
            end do
            if (.not. all(ieee_is_finite(estimate))) first_active_success = .false.
        end if
        if (first_active_success) then
            do i = 1, m
            ...
            accepted_beta = estimate
            accepted_beta_bits = transfer(accepted_beta, accepted_beta_bits)
            first_n_sub_sorted(rd) = n_active
            repair_count_sorted(rd) = 0
            final_n_sub_sorted(rd) = n_active
            init_mode_sorted(rd) = 0
            init_trigger_sorted(rd) = 0
            first_n_sub_out(z_order(rd)) = n_active
            repair_count_out(z_order(rd)) = 0
            final_n_sub_out(z_order(rd)) = n_active
            goto 41000
        end if
    end if
...
41000 continue
    do rd = 2, rounds
        ...
        attempt_loop: do while (not_optimal)
            ...
            ! Candidate residual aliases are attempt-local borrowed views.
            ! Reset before any rebuild/recovery work can reuse scratch state.
            r_candidate => r_raw
            candidate_reuses_accepted_residual = .false.
            ...
            if (not_new_sl_sh) then
                ...
                current_threshold_bits = transfer(M_threshold, current_threshold_bits)
                reuse_screen_topology = .false.
                if (screen_topology_cache_valid .and. case_int == 1 .and. n_active == m .and. &
                    (.not. threshold_lower_bound) .and. (.not. use_independent_init) .and. &
                    current_threshold_bits == cached_threshold_bits) then
                    reuse_screen_topology = &
                        ((H_prev(1) == cached_screen_H(1)) .and. &
                         (H_prev(2) == cached_screen_H(2))) .or. &
                        ((H_prev(1) == cached_screen_H(2)) .and. &
                         (H_prev(2) == cached_screen_H(1)))
                end if

                ! The cached arrays live in the same sl/sh storage and are a
                ! one-shot input to this evaluation point.  Consume the cache
                ! now so a repair retry can never reinterpret mutated tags as
                ! a fresh classification.
                screen_topology_cache_valid = .false.
                if (reuse_screen_topology) then
                    n_topology_reuses_local = n_topology_reuses_local + 1
                else
                    target_min = min(min_subsample_size, n_active)
                    min_k = 0.0d0
                    n_potential_S = 0
                    if (case_int == 2) then
                        do active_pos = 1, n_active
                            i = active_ids(active_pos)
                    ...
                    ! Classify observations in the reference row order.
                    if (case_int == 2) then
                        do active_pos = 1, n_active
                            i = active_ids(active_pos)
            ...
            ! Collect the at most q previous-H rows outside the exact active
            ! set and insertion-sort them by original observation ID.  Merging
            ! them with active_ids below reproduces the former 1:m row order.
            n_forced_h = 0
            if (.not. use_independent_init) then
                do i = 1, nvar+1
                    insert_id = H_prev(i)
                    if (.not. active(insert_id)) then
                        duplicate_h = .false.
                        do j = 1, n_forced_h
                            if (forced_h_ids(j) == insert_id) duplicate_h = .true.
                        end do
                        if (.not. duplicate_h) then
                            n_forced_h = n_forced_h + 1
                            insert_pos = n_forced_h
                            do while (insert_pos > 1 .and. &
                                      forced_h_ids(insert_pos-1) > insert_id)
                                forced_h_ids(insert_pos) = forced_h_ids(insert_pos-1)
                                insert_pos = insert_pos - 1
                            end do
                            forced_h_ids(insert_pos) = insert_id
                        end if
                    end if
                end do
            end if

            ! Build retained rows and exact aggregates in the same original-ID
            ! order as v33, without visiting inactive non-H observations.
            j = 0
            sum_w_sl = 0.0d0
            sum_w_sh = 0.0d0
            gammaxs_temp(m+1, :) = 0.0d0
            gammaxs_temp(m+2, :) = 0.0d0
            bs_temp(m+1) = 0.0d0
            bs_temp(m+2) = 0.0d0
            if (case_int == 2) then
                active_pos = 1
                forced_pos = 1
                do while (active_pos <= n_active .or. forced_pos <= n_forced_h)
                    if (active_pos > n_active) then
                        next_id = forced_h_ids(forced_pos)
                        forced_pos = forced_pos + 1
                    else if (forced_pos > n_forced_h) then
                        next_id = active_ids(active_pos)
                        active_pos = active_pos + 1
                    else if (active_ids(active_pos) < forced_h_ids(forced_pos)) then
                        next_id = active_ids(active_pos)
                        active_pos = active_pos + 1
                    else
                        next_id = forced_h_ids(forced_pos)
                    ...
                    if (.not. use_independent_init) then
                        if (i == H_prev(1) .or. i == H_prev(2)) not_jl_or_jh(i) = .true.
                    end if
                    if (not_jl_or_jh(i)) then
                        ...
                        gammaxs_temp(m+1, 2) = gammaxs_temp(m+1, 2) + x(i) * w(i)
                        bs_temp(m+1) = bs_temp(m+1) + y(i) * w(i)
                    else if (sh(i)) then
                        sum_w_sh = sum_w_sh + w(i)
                        gammaxs_temp(m+2, 2) = gammaxs_temp(m+2, 2) + x(i) * w(i)
            ...
            gammaxs_temp(m+1, 1) = sum_w_sl
            gammaxs_temp(m+2, 1) = sum_w_sh
            n_subsample = j
            ms = n_subsample
        ...
        if (use_independent_init) then
            ...
            if (.not. shifted_init_success) then
                ...
                if (always_same_h_refit) then
                    ...
                    if (.not. refit_recertified) then
                        call materialize_candidate_residuals(estimate)
                    end if
                else
                    call materialize_candidate_residuals(estimate)
                end if
                accept_subsample = certify_llqr_candidate(H_candidate, r_candidate, A, m, nvar, res_tol)
                ...
                final_n_sub_out(z_order(rd)) = m
                H_mat_sorted(rd, :) = H_candidate
                call accept_candidate_residuals()
            ...
            if (case_int == 2) then
                do active_pos = 1, n_active
                    i = active_ids(active_pos)
                    if ((sh(i) .and. r_candidate(i) <= 0.0d0) .or. &
                        (sl(i) .and. r_candidate(i) >= 0.0d0)) then
            ...
            if (n_bad_signs > 0) then
                ...
                if (dble(n_bad_signs) > 0.1d0 * dble(ms)) then
                    ...
                    if (case_int == 2) then
                        do active_pos = 1, n_active
                            i = active_ids(active_pos)
                            if (sh(i) .and. r_candidate(i) <= 0.0d0) sh(i) = .false.
                            if (sl(i) .and. r_candidate(i) >= 0.0d0) sl(i) = .false.
                        end do
                    else
                        do i = 1, m
                            if (sh(i) .and. r_candidate(i) <= 0.0d0) sh(i) = .false.
            ...
            do i = 1, n_subsample
                ...
                if (.not. is_in_H) then
                    ...
                    if (r(r_idx) > 0.0d0) then
                        n_idpos = n_idpos + 1
                        idpos(n_idpos) = i
                    else if (r(r_idx) < 0.0d0) then
                        n_idneg = n_idneg + 1
                        idneg(n_idneg) = i
                    else
                    end if
...
!DEBUG            end if
            ...
            if (has_sl_agg .or. has_sh_agg) then
                n_bad_signs = 0
                if (case_int == 2) then
                    do active_pos = 1, n_active
                        i = active_ids(active_pos)
...
!DEBUG            end if
            ...
            n_unmatched_pos = 0
            do i = 1, n_idpos
                row_map_pos(i) = pos_cache_slot(idx_Hbar_pos2(i))
                matched_rows_pos(i) = (row_map_pos(i) > 0)
                if (matched_rows_pos(i)) n_matched_pos = n_matched_pos + 1
...
!DEBUG                    end if
            ...
            n_unmatched_neg = 0
            do i = 1, n_idneg
                row_map_neg(i) = neg_cache_slot(idx_Hbar_neg2(i))
                matched_rows_neg(i) = (row_map_neg(i) > 0)
                if (matched_rows_neg(i)) n_matched_neg = n_matched_neg + 1
...
contains
    ...
    subroutine accept_candidate_residuals()
        implicit none
        integer :: ci

        screen_topology_cache_valid = candidate_reuses_accepted_residual .and. &
            case_int == 1 .and. n_active == m .and. (.not. threshold_lower_bound) .and. &
            repair_count_sorted(rd) == 0 .and. ieee_is_finite(M_threshold)
        if (screen_topology_cache_valid) then
            cached_threshold_bits = transfer(M_threshold, cached_threshold_bits)
            cached_screen_H = H_candidate
        end if

        if (candidate_reuses_accepted_residual) then
            n_cache_hit_accepts_local = n_cache_hit_accepts_local + 1
        else
            residual_swap => r
            r => r_raw
            r_raw => residual_swap
        ...
        do ci = 1, nvar + 1
            r(H_candidate(ci)) = 0.0d0
        end do
        accepted_beta = estimate
        accepted_beta_bits = transfer(accepted_beta, accepted_beta_bits)
        r_candidate => r
        candidate_reuses_accepted_residual = .false.
    end subroutine accept_candidate_residuals

    subroutine materialize_candidate_residuals(theta)
        implicit none
        double precision, intent(in) :: theta(nvar+1)
        integer :: ci

        r_candidate => r_raw
        candidate_reuses_accepted_residual = .false.
        do ci = 1, m
            r_raw(ci) = y(ci) - (A(ci, 1) * theta(1) + A(ci, 2) * theta(2))
        end do
    end subroutine materialize_candidate_residuals
    ...
    subroutine store_current_cache()
        ...
        do si = 1, n_pos_prev
            pos_cache_slot(idx_Hbar_pos(si)) = 0
        end do
        do si = 1, n_neg_prev
            neg_cache_slot(idx_Hbar_neg(si)) = 0
        end do
        n_pos_prev = 0
        n_neg_prev = 0
        do si = nvar+2, sms_org
            if (IBs(si) > nvar + 1 + ms) then
                ...
                neg_cache_slot(idx_Hbar_neg(n_neg_prev)) = n_neg_prev
                do sj = 1, nvar+1
                ...
                pos_cache_slot(idx_Hbar_pos(n_pos_prev)) = n_pos_prev
                do sj = 1, nvar+1
    ...
    subroutine try_same_h_refit(recertified)
        ...
        logical :: same_h_zero_mask, finite_refit
        integer :: ri, rj

        recertified = .false.
        n_refit_attempts_local = n_refit_attempts_local + 1
        ...
        trial_beta_bits = transfer(estimate_refit, trial_beta_bits)
        finite_refit = all(ieee_is_finite(estimate_refit))
        same_h_zero_mask = &
            ((H_candidate(1) == H_prev(1)) .and. (H_candidate(2) == H_prev(2))) .or. &
            ((H_candidate(1) == H_prev(2)) .and. (H_candidate(2) == H_prev(1)))

        ! The committed residual buffer has its accepted H entries forced to
        ! exact zero.  Therefore the reuse key includes both coefficient bits
        ! and the H zero-mask, not just the mathematical coefficient vector.
        if (finite_refit .and. same_h_zero_mask .and. &
            all(trial_beta_bits == accepted_beta_bits)) then
            r_candidate => r
            candidate_reuses_accepted_residual = .true.
            n_beta_cache_hits_local = n_beta_cache_hits_local + 1
            n_residual_evals_avoided_local = n_residual_evals_avoided_local + 1
        else
            call materialize_candidate_residuals(estimate_refit)
        end if

        if (.not. certify_llqr_candidate(H_candidate, r_candidate, A, m, nvar, res_tol)) return
