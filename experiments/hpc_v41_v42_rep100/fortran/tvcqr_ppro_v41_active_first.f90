...
! RESEARCH-ONLY v41: v39 plus an active-only cold solve at the first point,
! fit-wide LAPACK inverse-workspace reuse, and fused retained-row/aggregate
! accumulation. Because the Epanechnikov positive-weight support on the regular
! time grid is one contiguous interval, hot loops over its exact endpoints omit
! redundant per-row active-mask checks; endpoint discovery still evaluates the
! production weight expression exactly.
...
! repair/recovery decisions are unchanged after the first point. Omitting
! exactly zero-weight rows preserves the pointwise weighted objective, but this
! branch requires clarification of the contract phrase "full sample" and is not
! a production candidate unless its numerical basis path is separately accepted.
!
...
subroutine tvcqr_seq_ppro_fortran(x, y, m, nvar, tau, h, h_factor, tol, maxit, &
    ...
    ! Every inverse in this kernel has the same q-by-q shape. Query LAPACK's
    ! preferred DGETRI workspace once and reuse it for the whole fit instead of
    ! allocating, querying, and freeing workspace at every evaluation point.
    xhinv = 0.0d0
    do i = 1, 2*(nvar+1)
    ...
    ! Research ablation: solve the complete positive-weight pointwise problem
    ! at s_1 without carrying the trailing exactly zero-weight constraints.
    ! The active interval begins at observation 1 on this regular grid.
    eva_t = 1
    ms_org = 0
    has_sl_agg = .false.
    has_sh_agg = .false.
    ...
    beta_prev = 0.0d0
    iter = 0
    call run_fresh_initialization(beta_prev, ms_org, ms, first_active_success)
    if (.not. first_active_success) then
        goto 60000
    end if

    ! Canonicalize the active solution on its returned H basis and form the
    ! complete residual image needed by the next point.
    call same_h_refit_tvcqr(H_indices, A, y, m, 2*(nvar+1), res_tol, &
                            estimate_refit, r_refit, same_h_ok)
    if (.not. same_h_ok) then
        goto 60000
    end if
    ...
    do j = 1, 2*(nvar+1)
        ...
        H_seq(1, j) = H_indices(j)
        if (H_seq(1, j) < 1 .or. H_seq(1, j) > m) then
    ...
    do i = 1, m
        r_prev(i) = r_refit(i)
        if (store_residual) residual_est((i - 1) * m + 1) = r_prev(i)
    end do
    it_num(1) = iter
    first_n_sub(1) = ms_org
    repair_count(1) = 0
    final_n_sub(1) = ms_org
    r_prev_full = .true.
    r_prev_valid_lo = 1
    r_prev_valid_hi = m
    r_prev_extra_count = 0
    ...
    ! If the positive-weight first-point problem cannot supply a valid q-row
    ! interpolation basis, retain the original full-m cold solve below.  This
    ! preserves the prior behavior for rank-deficient or extremely narrow
    ! boundary windows without calling the sequential solver.
60000 continue
