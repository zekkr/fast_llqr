...
summarize_fit <- function(method, position, elapsed, estimate, raw = NULL,
  ...
  dimension_ok <- length(estimate) == expected_k
  ...
  list(
    ...
    elapsed_sec = as.numeric(elapsed),
    estimate = estimate,
    method_ok = isTRUE(kernel_ok && finite_estimate && dimension_ok),
    ...
    error_message = if (!dimension_ok) "dimension_mismatch" else if (!finite_estimate) "nonfinite_estimate" else NA_character_,
    threw_error = FALSE,
...
source(file.path(experiment_dir, "retry_helpers.R"))
settings <- retry_settings()
if (model != "llqr" && settings$policy != "none") stop("Baseline regeneration is LLQR-only")
...
run_attempt <- function(rep_id, seed) {
  ...
  generation_elapsed <- proc.time()[["elapsed"]] - generation_started
  ...
  rows <- lapply(base_methods, function(method) {
    ...
    data.frame(
      ...
      threw_error = item$threw_error,
      error_stage = item$error_stage,
      data_generation_sec = generation_elapsed,
...
run_replication <- function(rep_id) {
  result <- run_baseline_retries(rep_id, seed_base, settings, run_attempt)
  rows <- result$final
  ...
  cat(sprintf("rep=%d original_seed=%d final_seed=%d attempts=%d final_ok=%s\n",
              rep_id, seed_base + rep_id, rows$seed[[1L]], rows$attempt[[1L]], all(rows$method_ok)))
