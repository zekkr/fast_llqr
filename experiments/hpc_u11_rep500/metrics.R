...
valid_h_path <- function(h,n,q) {
  if (!is.numeric(h) || length(h)!=n*q || any(!is.finite(h)) ||
      any(h!=floor(h)) || any(h<1L | h>n)) return(FALSE)
  z <- matrix(as.integer(h),nrow=n,ncol=q)
  all(vapply(seq_len(n),function(i)length(unique(z[i,]))==q,logical(1L)))
}

audit_h_path <- function(method,solver_ok,candidate_h,lean_ok,lean_h,n,q) {
  ...
  if(!valid_h_path(lean_h,n,q))return(out("invalid_lean_h"))
  if(!valid_h_path(candidate_h,n,q))return(out("invalid_candidate_h"))
  a<-matrix(as.integer(candidate_h),n,q);b<-matrix(as.integer(lean_h),n,q)
  bad<-which(!vapply(seq_len(n),function(i)identical(sort.int(a[i,]),sort.int(b[i,])),logical(1L)))
...
empty_fit <- function(method,position,elapsed=NA_real_,message=NA_character_,stage="fit") {
  list(method=method,position=as.integer(position),elapsed_sec=as.numeric(elapsed),
       estimate=NULL,H=NULL,solver_ok=FALSE,finite_estimate=FALSE,ierr=NA_integer_,
