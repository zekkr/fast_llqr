...
source("reproduction/frozen/metrics.R")
...
# Capture actual worker environment, outside all fitting timers.
meta_dir <- file.path(output_root, run_tag, "_run_meta", "workers")
dir.create(meta_dir, recursive = TRUE, showWarnings = FALSE)
meta <- list(session = sessionInfo(), system = Sys.info(), cpu = cpu_model,
             RNGkind = RNGkind(), baseline_body = deparse(body(llqr_local_fit)),
             baseline_md5 = unname(tools::md5sum("R/llqr_functions.R")),
             cache_flags = Sys.getenv("SSQR_CACHE_FLAGS"), provider_flags = Sys.getenv("SSQR_PROVIDER_FLAGS"),
             thread_env = Sys.getenv(c("OMP_NUM_THREADS", "OPENBLAS_NUM_THREADS", "MKL_NUM_THREADS")))
if (Sys.getenv("SSQR_REQUIRE_HPC", "0") == "1") {
  stopifnot(getRversion() == "4.3.1", packageVersion("quantreg") == "5.94",
            grepl("6258R", cpu_model), all(meta$thread_env == "1"))
}
saveRDS(meta, file.path(meta_dir, sprintf("c%d_t%02d_n%d_task%d_%s.rds", case_id, round(100*tau), n, task_id, node_name)))
...
run_method <- function(method, position, dat) {
  ...
  tryCatch({
    raw <- if (model == "llqr") {
      h <- llqr_default_bandwidth(dat$x, dat$y, tau, h = NULL, case = 2L, h.factor = 1)
      switch(method,
        direct_baseline = llqr_local_fit(dat$x, dat$y, tau, dat$z, h, 2L, 1),
        lean_seq = run_llqr_lean(lean_dll$llqr, dat$x, dat$y, dat$z, tau, h, 2L),
        unified_u11 = ssqr_llqr(core, dat$x, dat$y, tau, 2L, h, dat$z, Mm.factor = 0.1)
      )
    } else {
      switch(method,
...
run_attempt <- function(rep_id, seed) {
  ...
  generated <- tryCatch({
    if (model == "llqr") {
      value <- if (case_id == 1L) generate_logistic_case2(n, seed) else {
        v <- generate_data(n, 2L, seed)
        c(v, list(z = sort(v$x), n_interior = sum(v$x >= .1 & v$x <= .9), placeholder = FALSE))
      }
      value
    } else {
  ...
  dataset_md5 <- NA_character_
  if (!inherits(generated, "error")) {
    tf <- tempfile(); saveRDS(generated, tf, compress = FALSE, version = 2)
    dataset_md5 <- unname(tools::md5sum(tf)); unlink(tf)
  }
  ...
  do.call(rbind, lapply(experiment_methods, function(method) {
    ...
    data.frame(
      run_tag, model, case = case_id, kernel_case = 2L, dataset_md5, tau, n, n_eval = ne,
...
if (nrow(status) != length(rep_ids) || !all(status$ok %in% TRUE)) quit(save = "no", status = 2L)
