...
stopifnot(nzchar(run_tag), nzchar(output_root), num_rep %in% c(1L, 500L), seed_base == 2025L)
...
if (nrow(u11_rep)) {
  selected <- u11_rep
  for (case_id in cases) for (tau in c(0.5, 0.2, 0.8)) for (model in models) for (n in ns) {
    d <- selected[selected$case == case_id & selected$model == model & selected$n == n & selected$tau == tau, , drop = FALSE]
    ...
    screen_rows[[sidx]] <- data.frame(
      model, case = unique(d$case), paper_case = case_id,
...
grid_complete <- length(missing_configs) == 0L && nrow(ii) == 24L && all(ii$complete %in% TRUE)
all_methods_complete <- nrow(summary) == 72L && all(summary$n_present == num_rep)
...
u11_gate <- nrow(u11) == 24L && all(u11$ierr_nonzero == 0L) &&
...
writeLines(Sys.getenv("SSQR_PUSHED_SHA"), file.path(run_dir, if(num_rep==1L) "PREFLIGHT_PASS" else "RESULTS_PASS"))
