...
Appendix~\ref{app:supplement}. We consider sample sizes $n\in\{200,500,1000,2000,5000\}$ and $1000$ Monte Carlo
...
The relation between \texttt{screen-seq(2)(ft)} and \texttt{seq(ft)} is more mixed. In the two LLQR cases it can
provide further reductions at some intermediate sample sizes, whereas in the two TVCQR cases it is slower than
\texttt{seq(ft)} for the sample sizes displayed. The overall pattern is consistent with warm starts providing the main
computational gain, while the effect of screening depends more strongly on the model setting.
...
\input{generated/runtime_summary_longtable.tex}
